function varargout = autocorr_test(y,numLags,numMA,numSTD)
...
%  Plot the sample ACF:
   ...
   lineHandles = stem(lags(2:end),acf(2:end),'filled','o','LineStyle','none');
...
%  null hypothesis is assumed to hold. 
   ...
   plot([numMA+0.5 numMA+0.5; numLags numLags],[bounds([1 1]) bounds([2 2])],'--k','linewidth',2);
   plot([0 numLags],[0 0],'-k','linewidth',2);
   hold('off')
   a = axis;
%    axis([a(1:3) 1]);
   axis([a(1:2) -.3 .3]);
